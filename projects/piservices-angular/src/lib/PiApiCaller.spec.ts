...
import { PiservicesAngularModule } from './piservices-angular.module';
import { PiTypeDescriptor, PiFieldDescriptor } from 'piservices-common';
...
const desc = new PiTypeDescriptor([
    ...
    new PiFieldDescriptor('id', 'integer', true),
    new PiFieldDescriptor('status', 'string[]', false),
    new PiFieldDescriptor('dates', 'date[]', false)
]);
...
function setup() {
    beforeEach(() => {
        TestBed.configureTestingModule({
            imports: [HttpClientTestingModule, PiservicesAngularModule]
        });
...
describe('GET tests validating parameters before send', () => {
    ...
    it('with unknown path parameters', () => {
        ...
        ).toThrowError(/Field descriptor for.*not found/)
    })
